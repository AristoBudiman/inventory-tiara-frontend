...
import { Info, AlertTriangle, CheckCircle, XCircle } from 'lucide-vue-next'
...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-9999 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in custom-scrollbar">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-slide-up border-t-8" :class="type === 'confirm' ? 'border-amber-500' : 'border-blue-500'">
      
      <div class="p-6 text-center">
        <div class="mx-auto w-16 h-16 rounded-full flex items-center justify-center mb-4" :class="type === 'confirm' ? 'bg-amber-100 text-amber-600' : 'bg-blue-100 text-blue-600'">
           <AlertTriangle v-if="type === 'confirm'" :size="32" />
           <Info v-else :size="32" />
        </div>
      ...
      <div class="p-4 bg-gray-50 flex gap-3 justify-center border-t border-gray-100">
        <template v-if="type === 'confirm'">
          ...
          <button @click="close(true)" class="px-5 py-2.5 rounded-xl font-bold text-white bg-amber-500 hover:bg-amber-600 shadow-md transition-all active:scale-95 w-full">Ya, Lanjutkan</button>
        </template>
        <template v-else>
